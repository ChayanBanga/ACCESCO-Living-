...
function init() {
    ...
    // Correctly set initial FOV for mobile
...
function update() {
    ...
    if(playerGroup) {
        ...
        // DYNAMIC BOUNDARIES: Narrower on mobile screens to prevent going off-screen
        const bounds = window.innerWidth < 600 ? 9.5 : 14;
        playerGroup.position.x = Math.max(-bounds, Math.min(bounds, playerGroup.position.x));

        const cfg = CAM_CONFIGS[state.cameraIdx];
        
        // INCREASED FOLLOW INTENSITY: Follow the car horizontally more on mobile
        const followIntensity = window.innerWidth < 600 ? 0.8 : 0.5;
        const ideal = cfg.offset.clone().add(new THREE.Vector3(playerGroup.position.x * followIntensity, 0, 0));
...
window.onresize = () => { 
    ...
    // RESPONSIVE FOV: Make the camera see more on narrow screens
    camera.fov = aspect < 1 ? 75 : 50;
